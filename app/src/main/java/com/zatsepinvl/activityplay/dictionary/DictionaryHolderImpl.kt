...
class DictionaryHolderImpl @Inject constructor(
    ...
    private var dictionaryLanguage: String? = null

    override fun loadDictionary(lang: String): Dictionary {
        if (dictionary == null || lang != dictionaryLanguage) {
            dictionaryLanguage = lang
            dictionary = context.assets.dictionary(
                WordFile("words/words_${lang}.txt")
            )
        }
        return dictionary!!
    }
